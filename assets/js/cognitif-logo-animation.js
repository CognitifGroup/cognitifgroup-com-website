...
(function () {
  ...
  function makeMark(cfg) {
    ...
    var render = function (ctx, W, H, te) {
      ...
      var lineAlpha = cfg.lineAlpha || (lightIcon ? 0.38 : 0.52);
      ...
      var bezel = (ic || cfg.frame === false) ? 0 : smooth(clamp((te - 0.62) / 0.7, 0, 1));
      if (bezel > 0.01) {
        ctx.lineWidth = 1;
      ...
      var hair = (ic || cfg.frame === false) ? 0 : smooth(clamp((te - 0.84) / 0.86, 0, 1));
      if (hair > 0.01) {
        ctx.lineWidth = 1;
      ...
      if (front > -0.09) {
        ...
        ctx.lineWidth = cfg.lineWidth || 1.15;
        ctx.beginPath();
    ...
    render.duration = cfg.duration || 3.35;
    return render;
  }
